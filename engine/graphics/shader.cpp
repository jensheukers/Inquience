...
std::string Shader::LoadShaderFile(std::string shaderPath) {
	std::string buildPath = Core::GetExecutableDirectoryPath();
	std::ifstream _file;
	_file.open(buildPath.append(shaderPath)); // Open the file

	if (!_file) { // If we cannot open
		Debug::Log("Cannot open vertex or fragment shader file, returning empty string"); // Log error
...
bool Shader::LoadShader(std::string vertexShaderPath, std::string fragmentShaderPath) {
	...
	if (!vsuccess)
	{
		Debug::Log("Vertex shader could not be compiled!");
		Debug::Log(vinfoLog);
		return 1;
	}
	...
	if (!fsuccess)
	{
		Debug::Log("Fragment shader could not be compiled!");
		Debug::Log(finfoLog);
		return 1;
	}
	...
	if (!psuccess)
	{
		...
		Debug::Log("Shader program could not be created!");
		Debug::Log(pinfoLog);
		return 1;
	}
	...
	Debug::Log("Shader (" + std::string(vertexShaderPath) + " - " + std::string(fragmentShaderPath) + ") Program created!");
...
Shader::Shader(std::string vertexShaderPath, std::string fragmentShaderPath) {
	...
	Debug::Log("Succesfully created shader program");
}

void Shader::Recompile() {
	if (!this->_shaderProgram) {
		Debug::Log("No shader program, cant recompile");
	}
	...
	if (this->LoadShader(this->_vertexShaderPath, this->_fragmentShaderPath) == 0) {
		Debug::Log("Failed to recompile Shader Program");
	}
}
